...
rotyON = 1;
gmON = 1;
saveFileName = 'Y-shaped_Frame_model4test_V2.stl';

lenUnit = 'mm';

topANDbot2sinkRadiiRatio = 3; % need > 1 | If it → +∞, then V2 → V1. 
...
sink = sizeInf.Y.thk(1)/(topANDbot2sinkRadiiRatio - 1);

% Sink Vertices and Faces
sinkVnum = 5;
sinkFace.Bevel = sinkVnum*((1:HHnum)'-1) + (1:4);
sinkFace.Level = sinkVnum*((1:HHnum)'-1) + (4:6);
sinkFace.Level(end, end) = 1;
sinkFace.LevelVec = reshape(sinkFace.Level', 1, []);

sinkVrtx = NaN(3, sinkVnum*HHnum);
sinkVrtxNum = size(sinkVrtx, 2);
normVec4Bevel = NaN(3, HHnum);
drctVec4Bevel = NaN(3, HHnum);
drctVec4Level = NaN(3, HHnum);
for n = 1:HHnum
    [sinkVrtx(:, sinkFace.Bevel(n, :)), normVec4Bevel(:, n), ...
        drctVec4Bevel(:, n), drctVec4Level(:, n)] = ...
        getBevel(baseP(:, n), cntrP(:, n), normVec(:, n), sizeInf.Y(n, :), sink);
end

for n = 1:HHnum
    inPid = mod([n, n+1], HHnum);
    inPid(inPid == 0) = HHnum;
    sinkVrtx(:, sinkFace.Level(n, 2)) = getLevel(sinkVrtx(:, ...
        [sinkFace.Level(n, 1), sinkFace.Level(n, end)]), ...
        drctVec4Level(:, inPid));
end

% Bottom Vertices and Faces
botVnum = sinkVnum + 4;
botFace.Bevel = botVnum*((1:HHnum)'-1) + (2:7);
botFace.Level = botVnum*((1:HHnum)'-1) + (7:11);
botFace.Level(end, end-1:end) = 1:2;
botFace.LevelVec = reshape(botFace.Level', 1, []);

botVrtx = NaN(3, botVnum*HHnum);
botVrtxNum = size(botVrtx, 2);
botVrtx(:, reshape(botFace.Level(:, 2:4)', 1, [])) = ...
    sinkVrtx(:, sinkFace.LevelVec) + sink.*[0, 0, 1]';
for n = 1:HHnum
    botVrtx(:, botFace.Bevel(n, 2:end-1)) = ...
        sinkVrtx(:, sinkFace.Bevel(n, :)) + sink.*normVec4Bevel(:, n);
    botVrtx(:, botFace.Bevel(n, 1)) = ...
        getCorner(botVrtx(:, botFace.Bevel(n, 1) - 1), ...
        [0, 0, 1]', normVec4Bevel(:, n), sink);
    botVrtx(:, botFace.Bevel(n, end)) = ...
        getCorner(botVrtx(:, botFace.Bevel(n, end) + 1), ...
        [0, 0, 1]', normVec4Bevel(:, n), sink);
end
...
topVrtx(:, reshape(topFace.Level(:, 2:4)', 1, [])) = ...
    sinkVrtx(:, sinkFace.LevelVec) + (sizeInf.Y.thk(1) + sink).*[0, 0, 1]';
for n = 1:HHnum
    topVrtx(:, topFace.Bevel(n, 2:end-1)) = ...
        sinkVrtx(:, sinkFace.Bevel(n, :)) + (sizeInf.Y.thk(1) + sink).*normVec4Bevel(:, n);
    topVrtx(:, topFace.Bevel(n, 1)) = ...
        getCorner(topVrtx(:, topFace.Bevel(n, 1) - 1), ...
        [0, 0, 1]', normVec4Bevel(:, n), sizeInf.Y.thk(1) + sink);
...
% Side Faces
sidFace.Corner = getSideFace(botFace.Bevel(:, [1, end]), topFace.Bevel(:, [1, end]), 'Corner4BluntEdge');
sidFace.Rectangle.Bevel = getSideFace(botFace.Bevel(:, 2:end-1), topFace.Bevel(:, 2:end-1), 'Rectangle');
sidFace.Rectangle.Level = getSideFace(botFace.Level(:, 2:end-1), topFace.Level(:, 2:end-1), 'Rectangle');
sidFace.Rectangle.Hole = getSideFace(flip(holFace.bot, 2), flip(holFace.top, 2), 'Rectangle');
sidFace.allRectangle = [
...
normVrtx1 = [
    ...
    2,  4,  2,  0,  0,  0;
    0,  0,  0,  0,  0,  0];
TriFace.sidCorner = getTriFace4S(normVrtx1, sidFace.Corner, 1, 0, 1);
normVrtx2 = [
    0,  1,  2,  1;
    0, -2,  0,  2;
    0,  0,  0,  0];
TriFace.sidRectangle = getTriFace4S(normVrtx2, sidFace.allRectangle, 1, 0, 1);
...
triFace = [
    ...
    cell2mat(TriFace.sidCorner)
    cell2mat(TriFace.sidRectangle)];
